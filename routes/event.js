...
const { updateStatus, getEvent, joinEvent, createEvent, expireEvent, cancelStatus, getEventPlayers } = require('../controller/event')
const asyncHandler = fn => (req, res, next) => Promise.resolve(fn(req, res, next)).catch(next);

eventRouter.get("/event", auth, asyncHandler(getEvent));
eventRouter.get("/event/detail/:id", auth, asyncHandler(getEventPlayers));
